...
namespace JAICT.Blazor.Recipes.UI.Client
{
    ...
    {
        public async Task<T> Get<T>(string key, IJSRuntime jsRuntime) where T : class
        {
            Debug.WriteLine("CookieStorage.Get");
            var exists =  await jsRuntime.InvokeAsync<bool>("cookieExists", key);
            if (exists)
            {
                Debug.WriteLine("CookieStorage.Get, ookie exists getting cookie");
                var result = await jsRuntime.InvokeAsync<string>("getCookie", key);
        ...
        public async Task Store<T>(T state, string key, IJSRuntime jsRuntime) where T :class
        {
            ...
            var result = await jsRuntime.InvokeAsync<bool>("setCookie", key, st);
            if (!result)
